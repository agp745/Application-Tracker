...
export function UpdateStatusDropdown({ currentStatus, id }: UpdateStatusProps) {
  ...
  const getIcon = () => {
    if (status === "pending")
      return (
        <ClockIcon className="w-5 h-5 text-yellow-600 hover:brightness-150 transition-all duration-100 ease-in" />
      );
    else if (status === "accepted")
      return (
        <CheckIcon className="w-5 h-5 text-green-600 hover:brightness-150 transition-all duration-100 ease-in" />
      );
    else if (status === "rejected")
      return (
        <XMarkIcon className="w-5 h-5 text-red-600 hover:brightness-150 transition-all duration-100 ease-in" />
      );
    else if (status === "ghosted")
      return (
        <GhostIcon className="w-5 h-5 text-neutral-200 hover:brightness-150 transition-all duration-100 ease-in" />
      );
